...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	int iterations = ROUNDUP(size,4096)/4096;
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	//TODO: [FINAL_EVAL_2020 - VER_C] - [2] USER HEAP [Kernel Side Free]
		...
		for(int i =0;i < e->page_last_WS_index;i++)
			{

				if(e->ptr_pageWorkingSet[i].virtual_address >= ROUNDDOWN(virtual_address,4096)
						&& e->ptr_pageWorkingSet[i].virtual_address < ROUNDUP(virtual_address + size,4096))
				{
				//	cprintf("unmaped va = %d \n",e->ptr_pageWorkingSet[i].virtual_address);
					//cprintf("Iam in WS !!!\n");
					unmap_frame(e->env_page_directory,(uint32*) e->ptr_pageWorkingSet[i].virtual_address);
		...
		//cprintf("va = %x \t va + size = %x \t size = %d \n",virtual_address,(virtual_address + size),size);
		//3. Removes ONLY the empty page tables (i.e. not used) (no pages are mapped in the table)
			while(virtual_address < virtual_address + size)
			{
			//cprintf("test4 \n");
				int count = 0;
				...
				if (ptr_page_table  !=  NULL)
				{
					...
					if(count == 1024)
					{
					//	cprintf("Iam in Free Table !!!\n");
						uint32 va =e->env_page_directory[PDX(virtual_address)];
						struct Frame_Info *frame = to_frame_info(va);
						frame->references = 0;
						free_frame(frame);
