...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	...
	int ret = allocate_frame(&ptr);
		...
		curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].time_stamp =0;
		...
		if (ret2 == E_PAGE_NOT_EXIST_IN_PF)
		{
			...
			{
				ret2 = pf_add_empty_env_page(curenv,fault_va,0);
				...
				if(ret2 == E_NO_PAGE_FILE_SPACE)
					panic("ERROR: No enough virtual space on the page file");

			}
			else
			{
				panic("this va doesn't exist");
			}
